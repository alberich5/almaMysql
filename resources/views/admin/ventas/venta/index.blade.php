...
<div class="row">
	<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
		...
		@include('admin.ventas.venta.search')
	</div>
</div>
<div id="mover">
			...
			<table class="table table-striped table-bordered table-condensed table-hover">
               ...
               @foreach ($ventas as $ven)
				<tr>
					...
					<td>{{ $ven->estado}}</td>
					<td>
						<a href="{{URL::action('admin\VentaController@show',$ven->idventa)}}"><button class="btn btn-primary">Detalle</button></a>
                         <a href="" data-target="#modal-delete-{{$ven->idventa}}" data-toggle="modal"><button class="btn btn-danger">Cancelar</button></a>
					</td>
				</tr>
				@include('admin.ventas.venta.modal')
				@endforeach
			</table>
		</div>
		{{$ventas->render()}}
